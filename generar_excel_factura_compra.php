...
header('Content-Disposition: attachment; filename="Factura_Compra_' . ($factura['numeroFactura'] ?? $factura['consecutivo']) . '_' . date('Y-m-d') . '.xls"');
                ...
                <td width="33%" style="vertical-align: top; padding: 0 10px;">
                    ...
                    <div class="section-content">
                        ...
                        <?php if ($factura['retenciones'] > 0): ?>
                            <div style="margin-top: 8px;">Retencion: <?= htmlspecialchars($factura['retencion_tarifa'] ?? '0') ?>%</div>
                        <?php endif; ?>
                    </div>
                </td>
        ...
        <table width="100%" border="1" cellpadding="10" cellspacing="0" style="border-collapse: collapse; margin: 20px 0;">
            ...
            <tbody>
                ...
                <tr style="border-bottom: 1px solid #dee2e6; <?= ($index % 2 == 0) ? 'background-color: #f8f9fa;' : '' ?>">
                    ...
                    <td style="padding: 10px; border: 1px solid #ddd; text-align: right;" class="amount">$<?= number_format($detalle['precioUnitario'], 2) ?></td>
                    <td style="padding: 10px; border: 1px solid #ddd; text-align: right;" class="amount">$<?= number_format($detalle['iva'], 2) ?></td>
                    <td style="padding: 10px; border: 1px solid #ddd; text-align: right;" class="amount">$<?= number_format($detalle['valorTotal'], 2) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
